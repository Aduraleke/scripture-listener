...
import { Mic, MicOff, Book, Download, Search, Trash2, RefreshCw, Lightbulb, Volume2, Maximize, Minimize, Wifi, WifiOff, Share2, MessageSquare, Bookmark, Map, FileText, Zap } from 'lucide-react';
...
const ScriptureListenerPro = () => {
  ...
  const translations = [
    ...
    { value: 'nkjv', label: 'NKJV - New King James Version', apiSupported: false },
    { value: 'niv', label: 'NIV - New International Version', apiSupported: false },
    { value: 'nlt', label: 'NLT - New Living Translation', apiSupported: false },
    { value: 'amp', label: 'AMP - Amplified Bible', apiSupported: false },
    { value: 'ampc', label: 'AMPC - Amplified Bible Classic', apiSupported: false },
    { value: 'cev', label: 'CEV - Contemporary English Version', apiSupported: false },
    { value: 'msg', label: 'MSG - The Message', apiSupported: false },
    { value: 'web', label: 'WEB - World English Bible', apiSupported: true },
    { value: 'clementine', label: 'Clementine Latin Vulgate', apiSupported: true },
    { value: 'almeida', label: 'João Ferreira de Almeida', apiSupported: true },
    { value: 'rccv', label: 'Romanian Corrected Cornilescu', apiSupported: true }
  ];
  ...
  const bibleBooks: Record<string, string> = useMemo<Record<string, string>>(() => ({
    ...
    '1 kings': '1 Kings', 'first kings': '1 Kings', '1kings': '1 Kings', '1ki': '1 Kings',
    '2 kings': '2 Kings', 'second kings': '2 Kings', '2kings': '2 Kings', '2ki': '2 Kings',
    ...
    '1 peter': '1 Peter', 'first peter': '1 Peter', '1pet': '1 Peter', '1pe': '1 Peter',
    '2 peter': '2 Peter', 'second peter': '2 Peter', '2pet': '2 Peter', '2pe': '2 Peter',
    '1 john': '1 John', 'first john': '1 John', '1joh': '1 John', '1jn': '1 John',
    '2 john': '2 John', 'second john': '2 John', '2joh': '2 John', '2jn': '2 John',
    '3 john': '3 John', 'third john': '3 John', '3joh': '3 John', '3jn': '3 John',
    'jude': 'Jude', 'jud': 'Jude',
  ...
  const updateHighlightedTranscript = useCallback((text: string) => {
    let highlighted = text;
    references.forEach(ref => {
      const regex = new RegExp(`\\b${ref.reference.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'gi');
      highlighted = highlighted.replace(regex, `<mark class="bg-purple-500/30 px-1 rounded">$&</mark>`);
    });
    setHighlightedTranscript(highlighted);
  }, [references]);
  
  const normalizeBookName = useCallback((name: string): string | null => {
    const normalized: string = name.toLowerCase().replace(/[^a-z0-9\s]/g, '').trim();
    ...
    patterns.forEach(pattern => {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        if (bookName) {
          ...
          const refString = `${bookName} ${chapter}:${verseStart}${verseEnd !== verseStart ? '-' + verseEnd : ''}`;
        ...
        rec.onerror = (event) => {
          ...
          switch(event.error) {
            ...
            case 'not-allowed':
              ...
              break;
            case 'network':
              errorMessage = 'Network error. Please check your internet connection or switch to offline mode.';
              break;
            default:
  ...
  const fetchVerse = async (ref: Reference): Promise<void> => {
    setLoading(true);
    setSelectedReference(ref);
    
    try {
      const verseQuery = `${ref.book} ${ref.chapter}:${ref.verseStart}${ref.verseEnd !== ref.verseStart ? '-' + ref.verseEnd : ''}`;
      const currentTranslation = translations.find(t => t.value === translation);
      
      // Check if translation is supported by bible-api.com
      if (currentTranslation?.apiSupported) {
        const apiUrl = `https://bible-api.com/${encodeURIComponent(verseQuery)}?translation=${translation}`;
        ...
        // Use AI to fetch verses for translations not in bible-api.com
        const aiResponse = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({
            ...
            messages: [{
              role: 'user',
              content: `Please provide the exact text of ${verseQuery} from the ${currentTranslation?.label} Bible translation. Return ONLY the verse text without any commentary, explanations, or additional text. Do not include the verse reference in the response.`
            }]
          })
        });

        const aiData = await aiResponse.json();
        const text = aiData.content.find((c: { type: string }) => c.type === 'text')?.text || 'Verse not available';
        setVerseText(text.trim());
      }
    } catch (error) {
      console.error('Error fetching verse:', error);
      setVerseText('Error loading verse. Please try again or switch translations.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateInsights = async (ref: Reference) => {
    ...
    try {
      ...
      const data = await response.json();
      const text = data.content.find((c: { type: string; text?: string }) => c.type === 'text')?.text || 'Insights not available';
      setInsights(text);
    } catch (error) {
...
Include: 1) Main theme, 2) Key points (3-4 bullets), 3) Practical applications`
      ...
      const data = await response.json();
      const text = data.content.find((c: { type: string; text?: string }) => c.type === 'text')?.text || 'Summary not available';
  ...
  const exportReferences = () => {
    let exportData = `SERMON NOTES\nDate: ${new Date().toLocaleString()}\nTranslation: ${translations.find(t => t.value === translation)?.label}\n\n`;
  ...
  return (
    <div className={`min-h-screen bg-linear-to-br from-slate-900 via-purple-900 to-slate-900 text-white ${fullscreen ? 'p-2' : 'p-4'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-800/50 backdrop-blur-lg rounded-2xl p-4 border border-slate-700/50">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xl font-bold text-purple-400">References</h2>
          ...
          {/* Verse Display Panel */}
          <div className="bg-slate-800/50 backdrop-blur-lg rounded-2xl p-4 border border-slate-700/50">
            <h2 className="text-xl font-bold text-purple-400 mb-3">Verse Text</h2>
            ...
            {loading ? (
              ...
              <div>
                <div className="mb-3 pb-3 border-b border-slate-700">
                  ...
                  <p className="text-xs text-slate-400 mt-1">{translations.find(t => t.value === translation)?.label}</p>
                </div>
        ...
        {!fullscreen && references.length > 0 && (
          <div className="mt-4 grid grid-cols-3 gap-3">
            <div className="bg-slate-800/50 backdrop-blur-lg rounded-xl p-3 border border-slate-700/50 text-center">
              <div className="text-2xl font-bold text-purple-400">{references.length}</div>
